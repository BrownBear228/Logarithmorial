...
$\frac{\partial{\sum_{k=1}^{n} {f_c}(k)}}{\partial{n}} =\sum_{k=1}^{n} \frac{\partial{{f_c}(k)}}{\partial{n}}$

На самом деле это довольно крамольная запись, по этому необходимо глубже разобраться в проблеме.

### Гамма-функция

Рассмотрим эту проблему на уже решенном кейсе - Гамма-функции.

Сама Гамма-функция является аналитическим продолжением факториала $n! = \prod_{k=1}^{n} k$
