...
${Log}(x) = \exp( \sum_{k=1}^{\infty} \bigl( \ln\ln(k+1) - \ln\ln(k+x) \bigr) + \sum_{k=1}^{\infty} \binom{x}{k} \Delta^{k-1} \ln\ln(N+1) ) ,$
...
Я увлёкся функциями вида ${f!}(x) = \prod_{k=1}^{n} {f_c}(k)$ и назвал эту конструкцию продукториалом (от product - умножение).
...
### Обобшение на произвольную операцию - Коммониал
