...
\[
${Log}(x) = \prod_{k=1}^{\infty} \ln(k+1) \quad\text{(для натуральных }x\text{)}$
\]
...
\[
\operatorname{Log}(n) = \prod_{k=1}^{n} \ln(k+1),\quad n\in\mathbb{N}.
\]

Для нецелых аргументов функция строится с помощью аналитического продолжения, основанного на представлении

\[
\ln\operatorname{Log}(x) = \sum_{k=1}^{\infty} \bigl( \ln\ln(k+1) - \ln\ln(k+x) \bigr) + \text{регуляризация},
\]
...
- Первая производная в нуле: \(\operatorname{Log}'(0) \approx -0.79468781007347122358197566427406854928\).
- Вторая производная в нуле: \(\operatorname{Log}''(0) \approx 1.92966\) (точнее задана через константу в коде).
